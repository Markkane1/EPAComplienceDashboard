...
import type { Application, HearingDate } from "@repo/shared";
import { applicationUseCases, hearingUseCases, userUseCases, violationUseCases } from "@/ui/app/container";
...
const Hearings = () => {
  ...
  const { data: hearings, isLoading } = useQuery<HearingRow[]>({
    queryKey: ["hearings"],
    queryFn: async () => hearingUseCases.list(),
  });
  ...
  const { data: applications } = useQuery<Application[]>({
    ...
    queryFn: async () =>
      applicationUseCases.list("status_in=complete,hearing_scheduled,under_hearing"),
  });
  ...
  const scheduleHearingMutation = useMutation({
    mutationFn: async () => {
      ...
      await applicationUseCases.scheduleHearing(selectedApplicationId, {
  ...
  const setViolationMutation = useMutation({
    mutationFn: async () => {
      return applicationUseCases.setViolation(selectedViolationAppId, {
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const result = await hearingUseCases.reportPdf(params);
                      const blob =
