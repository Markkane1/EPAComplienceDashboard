...
import { applicationUseCases, documentUseCases, reportUseCases } from "@/ui/app/container";
...
const Applications = () => {
  ...
  const { data: applicationsResponse, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return applicationUseCases.list(queryString);
    },
  });
  ...
  const markIncompleteMutation = useMutation({
    mutationFn: async () => {
      ...
      await applicationUseCases.markIncomplete(selectedApp.id, incompleteRemark.trim());
    },
    onSuccess: () => {
  ...
  const markCompleteMutation = useMutation({
    mutationFn: async () => {
      if (!selectedApp) return;
      await applicationUseCases.markComplete(selectedApp.id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["applications"] });
      queryClient.invalidateQueries({ queryKey: ["dashboard-stats"] });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return reportUseCases.registrarDisposal(params);
    },
  });
  ...
  const { data: remarks, isLoading: isRemarksLoading } = useQuery({
    ...
    queryFn: async () => {
      return applicationUseCases.listRemarks(String(selectedApp?.id));
    },
  });
  const actionItems = (() => {
  ...
  const handleDownload = useCallback(async (doc: ApplicationDocument) => {
    try {
      const blob = await documentUseCases.download(doc.id);
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      try {
        const data = await applicationUseCases.listDocuments(selectedApp.id);
        if (!isMounted) return;
