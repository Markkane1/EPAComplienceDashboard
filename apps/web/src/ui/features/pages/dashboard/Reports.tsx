...
import { reportUseCases, violationUseCases } from "@/ui/app/container";
...
const Reports = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return reportUseCases.summary(params);
    },
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (violationType) params.set("violation_type", violationType);
      if (subViolation) params.set("sub_violation", subViolation);
      return reportUseCases.hearingOfficerWise(params);
    },
  });
  ...
  const handleDownload = async () => {
    ...
    if (violationType) params.set("violation_type", violationType);
    if (subViolation) params.set("sub_violation", subViolation);
    const blob = await reportUseCases.summaryPdf(params);
  ...
  const handleHearingOfficerDownload = async (officerId: string | null) => {
    ...
    if (violationType) params.set("violation_type", violationType);
    if (subViolation) params.set("sub_violation", subViolation);
    const blob = await reportUseCases.hearingOfficerWisePdf(params);
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
