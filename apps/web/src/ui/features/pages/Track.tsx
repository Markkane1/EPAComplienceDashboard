...
import { trackingIdSchema, TrackingIdData } from "@/ui/app/validations";
...
import type { HearingInfo, TrackedApplication } from "@repo/shared";
import { toast } from "sonner";
import { withTimeout } from "@/ui/app/async";
import { publicUseCases } from "@/ui/app/container";
  ...
  const onSubmit = async (data: TrackingIdData) => {
    ...
    try {
      ...
      const appResult = await withTimeout(
        publicUseCases.getApplication(trackingId),
        10000,
      ...
      if (appResult && appResult.length > 0) {
        ...
        const hearingsResult = await withTimeout(
          publicUseCases.getHearings(trackingId),
          10000,
