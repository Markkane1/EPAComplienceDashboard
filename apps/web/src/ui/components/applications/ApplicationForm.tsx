...
import { applicationFormSchema, ApplicationFormData } from "@/ui/app/validations";
...
import type { Application, ApplicationDocument } from "@repo/shared";
import { useSearchParams } from "react-router-dom";
import { applicationUseCases, categoryUseCases } from "@/ui/app/container";
...
export function ApplicationForm() {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        const data = await categoryUseCases.list();
        if (isMounted) {
          setCategories(data || []);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadIncomplete = async () => {
      ...
      try {
        const data = await applicationUseCases.listIncomplete();
        if (!isMounted) return;
    ...
    const loadExisting = async () => {
      try {
        const [app, docs] = await Promise.all([
          applicationUseCases.getById(selectedExistingId),
          applicationUseCases.listDocuments(selectedExistingId),
        ]);
        if (!isMounted) return;
  ...
  const uploadDocuments = async (applicationId: string) => {
    ...
    const uploads = docTypes.map(async (docType) => {
      ...
      const uploaded = await applicationUseCases.uploadDocument(applicationId, formData);
  ...
  const onSubmit = async (data: ApplicationFormData) => {
    ...
    try {
      ...
      const application = isEditing
        ? await applicationUseCases.update(selectedExistingId, payload)
        : await applicationUseCases.create(payload);
