...
import HearingDate from "../../../infrastructure/db/mongoose/models/HearingDate.js";
...
import { mapHearing } from "../../../infrastructure/db/mongoose/mappers/mappers.js";
...
router.get("/", async (req, res) => {
  ...
  const hearings = await HearingDate.find({ application_id: { $in: applicationIds } })
    .lean()
    .sort({ hearing_date: 1 });
...
const buildHearingReportRows = async ({ from, to, officer_id, user }) => {
  ...
  const hearings = await HearingDate.find(hearingQuery).lean().sort({ hearing_date: 1 });
  ...
  const allHearings = hearings; // Reuse first query
