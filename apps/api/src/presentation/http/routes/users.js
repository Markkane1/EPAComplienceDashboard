...
import { mapUser } from "../../../infrastructure/db/mongoose/mappers/mappers.js";
import { logAudit } from "../../../infrastructure/services/audit.js";
import { validatePasswordStrength } from "../validators/schemas.js";
...
router.get("/hearing-officers", async (req, res) => {
  ...
  const officers = await User.find({ roles: "hearing_officer" }).lean().sort({ full_name: 1, created_at: -1 });
...
router.get("/", async (req, res) => {
  ...
  const users = await User.find(query).lean().sort({ created_at: -1 });
...
router.post("/", async (req, res) => {
  ...
  const existingByCnic = await User.findOne({ cnic: normalizedCnic }).lean();
  if (existingByCnic) {
...
router.post("/:id/reset-password", async (req, res) => {
  ...
  try {
    validatePasswordStrength(String(new_password));
  } catch (error) {
    return res.status(400).json({ message: error.message });
  }
