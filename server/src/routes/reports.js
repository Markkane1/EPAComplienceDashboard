...
const buildHearingOfficerReport = async ({ from, to, violationType, subViolation, user }) => {
  ...
  const applications = await Application.find(baseQuery).sort({ created_at: 1 });
...
router.get("/registrar-disposal", authRequired, async (req, res) => {
  ...
  const applications = await Application.find(appQuery).sort({ closed_at: -1 });
...
const buildSummary = async ({ from, to, violationType, subViolation, user }) => {
  ...
  const applications = await Application.find(baseQuery).select("status description");
...
router.get("/summary/pdf", authRequired, async (req, res) => {
  const { from, to, violation_type, sub_violation } = req.query;
  if (!from || !to) {
    return res.status(400).json({ message: "from and to are required." });
  }
...
router.get("/hearing-officer-wise", authRequired, async (req, res) => {
  const { from, to, violation_type, sub_violation } = req.query;
  if (!from || !to) {
    return res.status(400).json({ message: "from and to are required." });
  }
...
router.get("/hearing-officer-wise/pdf", authRequired, async (req, res) => {
